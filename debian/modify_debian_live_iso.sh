...
if [ $skip_fs -ne 1 ]; then
  ...
  if [ $skip_remove -ne 1 ]; then
    sudo chroot new_root apt autoremove --purge -y bluez bluez-firmware bluez-obexd calamares cups-browsed debian-reference-common exim4-base firmware-ivtv firmware-netronome fonts-thai-tlwg fonts-nanum fonts-noto-cjk-extra fonts-noto-extra fonts-noto-ui-extra fortunes-debian-hints gnome-games gnome-online-accounts gnome-initial-setup gnome-music gnome-software gnome-sushi gnome-themes-extra mlterm mlterm-tiny mlterm-tools pinentry-gnome3 rhythmbox shotwell totem vlc-l10n wnorwegian wpolish xiterm+thai yelp
