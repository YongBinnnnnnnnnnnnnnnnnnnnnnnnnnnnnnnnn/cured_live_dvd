...
if [ $skip_fs -ne 1 ]; then
  ...
  if [ $skip_remove -ne 1 ]; then
    sudo chroot new_root apt autoremove --purge -y avahi-daemon avahi-autoipd bluez bluez-firmware bluez-obexd calamares cups-browsed debian-reference-common exim4-base fonts-thai-tlwg fonts-nanum fonts-noto-cjk-extra fonts-noto-extra fonts-noto-ui-extra fortunes-debian-hints gnome-games gnome-online-accounts gnome-initial-setup gnome-music gnome-software gnome-sushi gnome-themes-extra maint-guide-it mlterm mlterm-tiny mlterm-tools plymouth pinentry-gnome3 rhythmbox shotwell totem vlc-l10n xiterm+thai yelp
    sudo chroot new_root apt autoremove --purge -y dahdi-firmware-nonfree firmware-ivtv firmware-netronome firmware-zd1211
    sudo chroot new_root apt autoremove --purge -y wbulgarian wngerman wnorwegian wpolish wspanish 
